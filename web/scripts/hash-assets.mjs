#!/usr/bin/env bun
// Rename the built JS, wasm and CSS to include a hash of their contents, and
// export those hashes for the Rust build.
//
// Two things depend on this. Hashed filenames can be cached forever without a
// stale asset surviving a deploy. And the Content-Security-Policy allows the
// inline hydration script by hash, which means the script — and therefore the
// filenames inside it — has to be settled before the header is built.
...
// A previous deploy's hashed files are still sitting in the output directory,
// and shipping them would serve whichever the asset handler found first.
...
async function main() {
  ...
  // The glue JS names the wasm file it loads, so renaming the wasm without
  // rewriting this leaves the page fetching a file that no longer exists.
    ...
    `${JSON.stringify(
      {
        ...
        hashes: { js: jsHash, wasm: wasmHash, css: cssHash },
      },
      null,
      2,
    )}\n`,
